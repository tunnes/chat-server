class Connection::Data::ConversationData
  def initialize(data)
    @data = data[:payload]
  end
  ...
  def initialize_conversation(conversation)
    attrs = { user_id: @data[:user_id], content: @data[:content], conversation_id: conversation.id }
    Message.create(attrs)
  end

  def save_conversation(type)
    conversation = Conversation.create(access_type: Conversation.statuses[type], title: @data[:title])
